...
class LNWatcher(PrintError):
    ...
    def write_to_disk(self):
        ...
        with self.lock:
            storage = self.synchronizer.storage
            # self.channel_info
    ...
    def check_onchain_situation(self, funding_outpoint):
        ...
        conf = self.synchronizer.get_tx_height(ctx_candidate_txid).conf
        # only care about confirmed and verified ctxs. TODO is this necessary?
        if conf == 0:
            return
        keep_watching_this = self.inspect_ctx_candidate(funding_outpoint, ctx_candidate)
    ...
    def inspect_ctx_candidate(self, funding_outpoint, ctx):
        ...
        for e_tx in encumbered_sweep_txns:
            conflicts = self.synchronizer.get_conflicting_transactions(e_tx.tx, include_self=True)
            ...
            if conflict_mined_status == TX_MINED_STATUS_FREE:
                tx_height = self.synchronizer.get_tx_height(ctx_txid).height
                num_conf = local_height - tx_height + 1
                if num_conf >= e_tx.csv_delay:
                    self.network.broadcast_transaction(e_tx.tx, self.print_tx_broadcast_result)
                else:
    ...
    def get_tx_mined_status(self, txid: str):
        ...
        tx_mined_status = self.synchronizer.get_tx_height(txid)
    ...
    def print_tx_broadcast_result(self, res):
        error = res.get('error')
        if error:
            self.print_error('tx broadcast failed: {}'.format(error))
        else:
            self.print_error('tx broadcast succeeded')
